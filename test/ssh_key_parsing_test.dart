...
void main() {
  ...
  test('an empty passphrase is rejected by dartssh2', () async {
    // The behaviour this guard exists for. If a future dartssh2 starts
    // accepting '' this test fails and the normalisation can be reconsidered
    // — but silently passing '' through must never come back.
    final key = await keygen.generate();
    expect(
  ...
  test('surrounding whitespace does not break loading', () async {
    ...
    final key = await keygen.generate();
    final padded = '\n  ${key.privateKeyPem}  \n';
    expect(() => SSHKeyPair.fromPem(padded.trim()), returnsNormally);
  });
}
